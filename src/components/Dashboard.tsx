...
import {
  Activity, Cpu, HardDrive, Zap, RefreshCw, ShieldCheck,
  ArrowDown, ArrowUp, Thermometer, CircuitBoard, Gauge as GaugeIcon,
} from 'lucide-react';
...
function StatCard({
  title, icon, accent = 'zinc', right, value, unit, barPct,
}: {
  title: string;
  ...
  const accentMap = {
    blue: { ring: 'hover:border-blue-500/30', iconBg: 'bg-blue-500/10 border-blue-500/20', iconText: 'text-blue-300', bar: 'bg-blue-500', glow: 'shadow-[0_0_30px_rgba(59,130,246,0.10)]' },
    purple: { ring: 'hover:border-purple-500/30', iconBg: 'bg-purple-500/10 border-purple-500/20', iconText: 'text-purple-300', bar: 'bg-purple-500', glow: 'shadow-[0_0_30px_rgba(168,85,247,0.10)]' },
    green: { ring: 'hover:border-green-500/30', iconBg: 'bg-green-500/10 border-green-500/20', iconText: 'text-green-300', bar: 'bg-green-500', glow: 'shadow-[0_0_30px_rgba(34,197,94,0.10)]' },
    zinc: { ring: 'hover:border-zinc-600/40', iconBg: 'bg-white/5 border-white/10', iconText: 'text-zinc-200', bar: 'bg-zinc-500', glow: '' },
  }[accent];

  return (
    <div className={[
      'flex-1 rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-md',
      'p-5 transition-all duration-300 shadow-lg shadow-black/30',
      accentMap.ring, accentMap.glow,
    ].join(' ')}>
      ...
      <div className="flex items-end gap-2 mb-3">
        <span className="text-4xl font-black text-white font-mono tracking-tight tabular-nums">{value}</span>
...
export default function Dashboard() {
  const [sysData, setSysData] = useState({
    ramUsed: '0', ramTotal: '16', cpuUsage: '0',
    netDown: '0.00', netUp: '0.00', cpuTemp: '--', gpuTemp: '--',
  });
  ...
  useEffect(() => {
    ...
    if (cpu > 80) score -= 20; else if (cpu > 50) score -= 10;
    const used = parseFloat(sysData.ramUsed) || 0;
    const total = parseFloat(sysData.ramTotal) || 16;
    const ramPct = (used / total) * 100;
    if (ramPct > 90) score -= 25; else if (ramPct > 70) score -= 15;
    const temp = parseInt(sysData.cpuTemp);
    if (!isNaN(temp)) {
      if (temp > 90) score -= 40; else if (temp > 80) score -= 20; else if (temp > 70) score -= 10;
    }
    if (!autoOptimize) score -= 5;
    setHealthScore(Math.min(100, Math.max(10, score)));
  ...
  return (
    // ✅ Issue 1 Fix: overflow-y-auto بدل overflow-hidden
    <div className="h-full flex flex-col bg-transparent overflow-y-auto overflow-x-hidden relative">

      {/* ✅ Inner wrapper مع min-h باش ما يتسحقوش العناصر */}
      <div className="flex flex-col gap-6 p-6 lg:p-8 min-h-[820px]">
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shrink-0">
          ...
          <div className="bg-white/[0.03] backdrop-blur-md border border-white/10 rounded-2xl p-4 shadow-lg shadow-black/30 flex items-center gap-4">
            ...
            >
              <div className={[
                'absolute top-1 left-1 w-6 h-6 rounded-full transition-all duration-300 bg-white shadow-md',
        ...
        {/* MAIN GRID — ✅ min-h-[400px] باش ما يطيحش */}
        <div className="grid grid-cols-12 gap-6 flex-1 min-h-[400px]">
            ...
            <div className="flex flex-col items-center justify-center h-full">
              <div className="relative w-72 h-44 sm:w-80 sm:h-48 flex-shrink-0">
                <svg className="w-full h-full overflow-visible relative z-10" viewBox="0 0 100 55">
                  <path d="M 10 50 A 40 40 0 0 1 90 50" fill="none" stroke="rgba(255,255,255,0.08)" strokeWidth="10" strokeLinecap="round" />
                  <path
                  ...
                  <g
                    className="transition-all duration-1000 ease-out"
                    style={{ transformOrigin: '50px 50px', transform: `rotate(${healthScore * 1.8 - 90}deg)` }}
                  >
                    <path d="M 50 50 L 50 16" stroke="white" strokeWidth="2.5" strokeLinecap="round" className="drop-shadow-lg" />
                ...
                <div className="absolute left-1/2 top-[62%] -translate-x-1/2 -translate-y-1/2 text-center z-20 pointer-events-none">
                  <span className="text-6xl font-black text-white italic tracking-tight drop-shadow-[0_0_18px_rgba(255,255,255,0.18)]">
                    {healthScore}<span className="text-2xl ml-1">%</span>
                  </span>
              ...
              <div className="mt-14 text-center space-y-4 w-full max-w-md">
                ...
                >
                  ...
                  {isOptimizing ? 'Optimizing...' : healthScore === 100 ? 'System Optimized' : 'Boost Performance'}
                </button>
          ...
          {/* STATS — ✅ min-w-[220px] باش ما يتضيّقوش */}
          <div className="col-span-12 lg:col-span-4 flex flex-col gap-4 min-w-[220px]">
            <StatCard
              title="CPU" accent="blue"
